...
def search():
    ...
    if query:
        # Use parameterized query with SQLAlchemy's text() function and bind parameters
        sql = text("SELECT * FROM student WHERE name LIKE :search_query")
        results = db.session.execute(sql, {'search_query': f'%{query}%'})
